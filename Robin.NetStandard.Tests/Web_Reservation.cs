    ...
    public async Task Get()
    {
        ...
        var client =
            (IRobinApi)new RobinClient(
                new HttpClient(
                    new ActionHandler(req => { Utility.ValidateApiCall(HttpMethod.Get, "reservations/seats?level_ids=103864&before=2024-01-09T08%3a30%3a00%2b00%3a00&after=2024-01-09T17%3a30%3a00%2b00%3a00&page=1&per_page=50", req); },
                        res)), "token");

        var response = await client.Reservations.Get(new GetReservationRequest
        {
            LevelIds = ["103864"],
